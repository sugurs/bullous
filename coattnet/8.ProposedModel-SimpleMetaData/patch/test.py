...
def testModelOnOneDataset(loaded_weights, test_data_dir):
    ...
    for file in tqdm(img_list):
        ...
        for each in samples:
            if lch_in_file == each['lch']:
                test_features.append(each['feature19'][:10])
    ...
    for each in LTPC_lch:
        for person in samples:
            if person['lch'] == each:
                features19 = person['feature19'][:10]
...
if __name__ == '__main__':
    ...
    # 找所有的hdf5
    hdf5_dir = '/media/wz209/a29353b7-1090-433f-b452-b4ce827adb17/sugurs/PythonProjects/pem_multi/Expirements/' \
               '8.ProposedModel-SimpleMetaData/patch/backup/'
    findH5Count = 0
    for fpathe, dirs, fs in os.walk(hdf5_dir):
        for f in fs:
            ...
            if file.endswith('.hdf5'):
                ...
                testDir = '/media/wz209/a29353b7-1090-433f-b452-b4ce827adb17/sugurs/PythonProjects/pem_multi/dataset/patch/%s' % (file.split('/')[-2])
